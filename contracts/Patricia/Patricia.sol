...
contract PatriciaTree {
    ...
    function getProof(bytes memory key) public view returns (uint branchMask, bytes32[] memory _siblings) {
        ...
        while (true) {
            ...
            _siblings[numSiblings++] = edgeHash(nodes[e.node].children[1 - head]);
    ...
    function verifyZeroKnowledgeProof(bytes32 rootHash, bytes memory key, bytes32 hashedValue, uint branchMask, bytes32[] memory siblings) public pure {
        D.Label memory k = D.Label(keccak256(key), 256);
        D.Edge memory e;
        e.node = hashedValue;
        for (uint i = 0; branchMask != 0; i++) {
        ...
        e.label = k;
        require(rootHash == edgeHash(e), "invalid-proof");
    }
